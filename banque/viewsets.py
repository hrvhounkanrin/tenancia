...
class BanqueViewSet(ActionAPIView):
    ...
    def create_banque(self, request, params={}, *args, **kwargs):
        ...
        if isinstance(request.data.get('banque', None), list):
            ...
            serialized_banques = BanqueSerializers(saved_banques, many=True, context=serializer_context)
    ...
    def update_banque(self, request, params={}, *args, **kwargs):
        ...
        if isinstance(request.data.get('banque', None), list):
            ...
            for banque in banques:
                ...
                serializer = BanqueSerializers(instance, data=banque, context=serializer_context)
                serializer.is_valid(raise_exception=True)
                serializer.save()
                return {"success": True, "banque": serializer.data}

        instance = Banque.objects.get(pk=params.get('id', None))
        serializer = BanqueSerializers(instance, data=request.data, context=serializer_context)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return {"success": True, "banque": serializer.data}
